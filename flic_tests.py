...
ax1 = ax.inset_axes([0.65, 0.5, 0.3, 0.4], xlim=(0.57, 0.69),
                    ylim=(0.185, 0.53), xticklabels=[], yticklabels=[])

limiters = ['minbee', 'vanleer', 'vanalbada', 'superbee']
lim_abbs = ['mb', 'vl', 'va', 'sb']

for lim, abb in zip(limiters, lim_abbs):
    hd = HDSolver1D(rho0, u0, p0, e0, dx, nt=nt, cfl_cut=cfl,
                    solver='flic', limit_func=lim)
...
fig.savefig(basepath + 'FLIC.png', bbox_inches='tight')
